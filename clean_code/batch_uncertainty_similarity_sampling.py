...
Fork from the modAL implementation that removes automatic alpha selection by a fixed ratio (usefull in a continuous context)
"""
...
def select_instance(
    ...
    """
    scores = calculate_scores(X_training, X_pool, X_uncertainty, mask, metric, alpha, n_jobs)
...
def ranked_batch(classifier: Union[BaseLearner, BaseCommittee],
    ...
    for _ in range(ceiling):
        ...
        instance_index, instance, mask = select_instance(X_training=labeled, X_pool=unlabeled,
                                                         ...
                                                         metric=metric, n_jobs=n_jobs, alpha=alpha)
...
def valuation(x_training: modALinput,
        x_pool: modALinput, # the thing to optimize
        x_uncertainty: np.ndarray,
        mask: np.ndarray,
        metric: Union[str, Callable],
        alpha: float,
    ...
    return -np.sum(calculate_scores(x_training, x_pool, x_uncertainty, mask, metric, alpha, n_jobs))

def uncertainty_batch_sampling_annealing(classifier: Union[BaseLearner, BaseCommittee],
    ...
    uncertainty = classifier_uncertainty(classifier, unlabeled, **uncertainty_measure_kwargs)

    # mask for unlabeled initialized as transparent
    #mask = np.ones(unlabeled.shape[0], bool)
    mask = np.ones(ceiling, bool)
    
    func = lambda x: valuation(x_training=labeled, x_pool=x, x_uncertainty=uncertainty, mask=mask, metric=metric, n_jobs=n_jobs, alpha=alpha)

    bounds = [[0, unlabeled.shape[0]] for _ in range(ceiling)]
...
def uncertainty_batch_sampling(classifier: Union[BaseLearner, BaseCommittee],
    ...
    Args:
        ...
        alpha: Ratio between similarity and uncertainty sampling (bigger alpha implies exploration is more important)
    ...
    return ranked_batch(classifier, unlabeled=X, uncertainty_scores=uncertainty, alpha=alpha,
